...
const Filter = (props) => {
    ...
    return (
        <div className="mx-auto max-w-[768px] flex items-center justify-center py-20">
            <div className="flex flex-col md:flex-row md:justify-between md:px-[30px] lg:px-6 gap-y-4 items-center w-[85%] max-w-[320px] md:max-w-none md:w-full">
                <button
                ...
                <button
                    className={`w-full md:w-[225px] lg:w-[230px] h-[75px] border border-primary text-[25px] ${selectedFilter === "non" ? "bg-primary text-white" : "bg-white text-primary"}`}
                    style={ {fontFamily: "'Bangers', cursive"} }
                    onClick={ () => {
                        dispatch(applyFilter('non'));
                    }}
                >
                    Non-veg pizzas
                </button>
